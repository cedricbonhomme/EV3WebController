...
from ev3webcontroller.decorators import to_response
from ev3webcontroller.lib import movements
from ev3webcontroller import app
from ev3webcontroller import right_wheel, left_wheel, button, ir_sensor, color_sensor
...
def move(direction="forward", speed=60):
    ...
    if direction == 'forward':
        ...
        if None is not nb_blocks:
            ...
            speed *= 2 # Robot was too slow for Élisabeth...
        ...
        if None is forever:
            movements.rotate(left_wheel, right_wheel, -360, 360, 180, 0)
        else:
            left_wheel.run_forever(speed * -1, regulation_mode=False)
        ...
        forever = request.args.get("forever", None)
        if None is forever:
            movements.rotate(left_wheel, right_wheel, 360, -360, 0, 180)
        else:
